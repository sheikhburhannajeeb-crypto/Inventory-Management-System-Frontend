                                ...
                                {showProductDropdown && (
                                    <div className="dropdown-options glass-panel" style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, maxHeight: '250px', overflowY: 'auto' }}>
                                        {products
                                            ...
                                            .filter(p => fuzzyMatch(productSearchTerm, p.name) || fuzzyMatch(productSearchTerm, String(p.id)) || fuzzyMatch(productSearchTerm, formatProductId(p.id)))
                                            .map(p => (
                                                <div
                                                    key={p.id}
                                                    className="dropdown-option"
                                        ...
                                        {products.filter(p => fuzzyMatch(productSearchTerm, p.name) || fuzzyMatch(productSearchTerm, String(p.id)) || fuzzyMatch(productSearchTerm, formatProductId(p.id))).length === 0 && (
